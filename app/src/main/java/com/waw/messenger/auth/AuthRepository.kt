...
data class AuthUser(val id: String, val username: String, val email: String, val displayName: String, val avatarUrl: String?)
...
data class SavedAccount(val id: String, val username: String, val displayName: String, val email: String, val token: String, val expiresAt: Long) {
    fun toJson() = JSONObject().apply { put("id", id); put("username", username); put("displayName", displayName); put("email", email); put("token", token); put("expiresAt", expiresAt) }
}
...
class AuthRepository(context: Context) {
    ...
    fun hasSession(): Boolean = !token().isNullOrBlank() && prefs.getLong("expires_at", 0L) > System.currentTimeMillis()
    fun token(): String? = prefs.getString("token", null)?.takeIf { prefs.getLong("expires_at", 0L) > System.currentTimeMillis() }
    ...
    fun savedAccounts(): List<SavedAccount> = runCatching {
        ...
        (0 until array.length()).map { index ->
            ...
            SavedAccount(item.getString("id"), item.getString("username"), item.getString("displayName"), item.getString("email"), item.getString("token"), item.getLong("expiresAt"))
    ...
    fun removeSavedAccount(accountId: String) {
        ...
        val array = org.json.JSONArray(); retained.forEach { array.put(it.toJson()) }
        secureStore.put("account_sessions", array.toString())
    }

    suspend fun register(username: String, email: String, password: String, displayName: String): AuthResult = requestAuth("/auth/register", JSONObject().apply { put("username", username); put("email", email); put("password", password); put("displayName", displayName) })
    suspend fun login(identifier: String, password: String): AuthResult = requestAuth("/auth/login", JSONObject().apply { put("identifier", identifier); put("password", password) })

    suspend fun requestPasswordReset(identifier: String): String = withContext(Dispatchers.IO) {
        val response = request("POST", "/auth/forgot-password", JSONObject().put("identifier", identifier), null)
    ...
    suspend fun resetPassword(resetToken: String, password: String) = withContext(Dispatchers.IO) {
        val response = request("POST", "/auth/reset-password", JSONObject().apply { put("token", resetToken); put("password", password) }, null)
        if (!response.isSuccessful) error(readError(response))
    }
    ...
    private suspend fun requestAuth(path: String, payload: JSONObject): AuthResult = withContext(Dispatchers.IO) {
        ...
        val user = parseUser(root.getJSONObject("user")); val sessionJson = root.getJSONObject("session")
    ...
    private suspend fun request(method: String, path: String, payload: JSONObject?, bearer: String?): okhttp3.Response {
    ...
    private fun rememberAccount(user: AuthUser, session: AuthSession) {
        ...
        val array = org.json.JSONArray(); accounts.take(5).forEach { array.put(it.toJson()) }
        secureStore.put("account_sessions", array.toString())
    }

    private fun parseUser(json: JSONObject) = AuthUser(json.getString("id"), json.getString("username"), json.getString("email"), json.getString("displayName"), json.optString("avatarUrl").ifBlank { null })
    private fun readError(response: okhttp3.Response): String {
        val raw = response.body?.string().orEmpty()
        return try { JSONObject(raw).optString("error").ifBlank { "HTTP_${response.code}" } } catch (_: Exception) { "HTTP_${response.code}" }
    }
}
